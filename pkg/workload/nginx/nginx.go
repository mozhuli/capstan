...
func (w *Workload) Run(kubeClient kubernetes.Interface) error {
	...
	for _, testingCase := range testingTool.GetTestingCaseSet() {
		...
		glog.V(1).Infof("Running the testing case %q of %v", testingCase.Name, w.GetName())
		...
		glog.V(4).Infof("Clean up all the resouces created by the testing case %q.", testingCase.Name)
		...
		glog.V(4).Infof("Sleep %v and start next testing case.", testingTool.GetSteps())
